...
Way::Way(const Way &way)
    : path_(way.path_), avgEdgeLen_(way.avgEdgeLen_), sizeToCar_(way.sizeToCar_) {
...
Tracklimits Way::getTracklimits() const {
  ...
  const Node *left = nullptr, *firstLeft = nullptr;
  const Node *right = nullptr, *firstRight = nullptr;
